 ...
 */
export function isKyuaruError(error) {
  return error instanceof KyuaruError;
}

/** @type {ReadonlyMap<number, { code: KyuaruErrorCode, message: string }>} */
const NATIVE_ERROR_TABLE = new Map([
  [-1, { code: "NATIVE_FAILURE", message: "Null pointer passed to native QR encoder" }],
  [-2, { code: "OUTPUT_TOO_SHORT", message: "Output buffer is too small" }],
  [-3, { code: "INVALID_ECC", message: "Invalid error correction level" }],
  [-4, { code: "INVALID_VERSION", message: "Invalid QR version range" }],
  [-5, { code: "INVALID_MASK", message: "Invalid mask; expected null or a value from 0 to 7" }],
  [-6, { code: "DATA_TOO_LONG", message: "Input data is too long for the requested QR versions and error correction level" }],
  [-7, { code: "INVALID_UTF8", message: "Text input is not valid UTF-8" }],
  [-8, { code: "INPUT_TOO_LONG", message: "Input exceeds the QR standard maximum length" }],
]);
 ...
 * @returns {KyuaruError}
 */
export function mapNativeError(nativeCode) {
  const entry = NATIVE_ERROR_TABLE.get(nativeCode);
  if (entry) return new KyuaruError({ ...entry, details: { nativeCode } });

  return new KyuaruError({
    code: "NATIVE_FAILURE",
    message: `Native QR encoder failed with code ${nativeCode}`,
    details: { nativeCode },
  });
}
